...
namespace UVP.ExternalIntegration.Business.Services
{
    public class ModelLoaderService : IModelLoaderService
    {
        ...
        public async Task<object> LoadModelDataAsync(string uvpDataModel, long integrationInvocationId)
        {
            ...
            // Extract IDs based on integration type
            int doaCandidateId = 0;
            int candidateId = 0;

            // EARTHMED-specific extraction from ReferenceNumber
            if (invocation.IntegrationType.Equals("EARTHMED", StringComparison.OrdinalIgnoreCase))
            {
                var (extractedDoaCandidateId, extractedCandidateId) = await ExtractIdsFromReferenceNumberAsync(keyBag);
                doaCandidateId = extractedDoaCandidateId;
                candidateId = extractedCandidateId;

                _logger.Information("[EARTHMED] Extracted from ReferenceNumber: DoaCandidateId={DoaCandidateId}, CandidateId={CandidateId}",
                    doaCandidateId, candidateId);
            }
            else
            {
                // CMTS and other integrations - use existing logic
                doaCandidateId = TryGetIntFromBag(keyBag, "externalBatchId");
                candidateId = TryGetIntFromBag(keyBag, "externalRequestId");
            }

            // For GET operations, try to extract ID from the request
            Guid id = Guid.Empty;
        ...
        public async Task<object> LoadModelDataAsync(string uvpDataModel, IntegrationRequestDto bootstrapRequest)
        {
            ...
            // 3) Build the dynamic model bundle
            return await BuildModelBundleAsync(uvpDataModel, doaCandidateId, candidateId);
        }

        /// <summary>
        /// Extract DoaCandidateId and CandidateId from EARTHMED ReferenceNumber format: DoaId_DoaCandidateId
        /// </summary>
        private async Task<(int doaCandidateId, int candidateId)> ExtractIdsFromReferenceNumberAsync(Dictionary<string, object> keyBag)
        {
            // Try to find ReferenceNumber in the keyBag
            ...
            {
                _logger.Warning("[EARTHMED] ReferenceNumber format invalid: {ReferenceNumber}", referenceNumber);
                return (0, 0);
            }
            ...
            if (parts.Length != 2)
            {
                _logger.Warning("[EARTHMED] ReferenceNumber does not contain exactly 2 parts: {ReferenceNumber}", referenceNumber);
                return (0, 0);
            }

            if (!int.TryParse(parts[0], out var doaId))
            {
                _logger.Warning("[EARTHMED] Failed to parse DoaId from ReferenceNumber: {Part}", parts[0]);
                return (0, 0);
            }

            if (!int.TryParse(parts[1], out var doaCandidateId))
            {
                _logger.Warning("[EARTHMED] Failed to parse DoaCandidateId from ReferenceNumber: {Part}", parts[1]);
                return (0, 0);
            }

            // Query DoaCandidate table to get CandidateId using both DoaId and DoaCandidateId
            var doaCandidate = (await _doaCandidateRepo.FindAsync(x => x.DoaId == doaId && x.Id == doaCandidateId))
                .FirstOrDefault();
            ...
            _logger.Information("[EARTHMED] Successfully extracted: DoaId={DoaId}, DoaCandidateId={DoaCandidateId}, CandidateId={CandidateId}",
        ...
        /// <summary>
        /// Build the dynamic model bundle based on requested models
        /// </summary>
        private async Task<object> BuildModelBundleAsync(string uvpDataModel, int doaCandidateId, int candidateId)
        {
            ...
            foreach (var name in modelNames)
            {
                ...
                switch (name)
                {
                    ...
                    case "User":
                        // Get the Candidate first to find the UserId
                        var candidateForUser = await _candidateRepo.GetByIdAsync(candidateId);
                        if (candidateForUser != null)
                        {
                            // FIXED: Use candidateForUser.UserId instead of candidateForUser.Id
                            var userEntity = await _userRepo.GetByIdAsync(candidateForUser.UserId);
                            if (userEntity != null)
                            {
                                modelData = new UserIntegrationModel
                                {
                                    ...
                                    EmailAddress = userEntity.PersonalEmail ?? string.Empty, // Get from User table
            ...
            switch (element.ValueKind)
            {
                case JsonValueKind.Object:
                    foreach (var prop in element.EnumerateObject())
                    {
                        ...
                        // Add ReferenceNumber for EARTHMED
                        if (name.Equals("ReferenceNumber", StringComparison.OrdinalIgnoreCase))
                        {
                            Add("ReferenceNumber", prop.Value);
                            Add(full, prop.Value);
                        }
...
//                // EARTHMED specific: Extract DoaId from ReferenceNumber
//                if (TryGetFromBag(keyBag, "ReferenceNumber", out var refNumObj))
//                {
//                    var refNum = refNumObj?.ToString();
//                    if (!string.IsNullOrWhiteSpace(refNum) && refNum.Contains("_"))
//                    {
//                        var parts = refNum.Split('_');
//                        if (parts.Length == 2 && int.TryParse(parts[0], out var extractedDoaId))
//                        {
//                            if (!keyBag.ContainsKey("DoaId"))
//                            {
//                                keyBag["DoaId"] = extractedDoaId;
//                                _logger.Debug("Extracted DoaId={DoaId} from ReferenceNumber", extractedDoaId);
//                            }
//                        }
//                    }
//                }
...
//                    //    break;
//                    case "Doa":
//                        // Try to get DoaId from keyBag first (for EARTHMED)
//                        int doaIdToUse = TryGetIntFromBag(keyBag, "DoaId");

//                        // Fallback to using doaCandidateId if DoaId not found
//                        if (doaIdToUse == 0)
//                        {
//                            doaIdToUse = doaCandidateId;
//                            _logger.Debug("DoaId not found in keyBag, using doaCandidateId={Id} as fallback", doaCandidateId);
//                        }

//                        var doaEntity = await _doaRepo.GetByIdAsync(doaIdToUse);
//                        if (doaEntity != null)
//                        {
//                            modelData = new DoaIntegrationModel
//                            {
...
//                                ExpectedEndDate = doaEntity.ExpectedEndDate
//                            };
//                            _logger.Debug("Loaded Doa entity with Id={DoaId}", doaIdToUse);
//                        }
//                        else
//                        {
//                            _logger.Warning("Doa entity not found for DoaId={DoaId}", doaIdToUse);
//                        }
//                        break;
